...
var BUTTON_POS_FIRE = {x:140, y:60, w:20, h:20};
var BUTTON_POS_ICE = {x:120, y:80, w:20, h:20};
...
var BUTTON_POS_UNLOCK = {x:160, y:80, w:20, h:20};
 ...
 * 2. Spells are ICEable (in order) so we have to skip unknown spells
 */
function action_logic_moveselect() {
  ...
  // currently on the run button
    ...
    if (pressing.left && !input_lock.left) {
      ...
      if (avatar.spellbook >= 2) {
        action.select_pos = BUTTON_POS_FIRE;
        input_lock.down = true;
        redraw = true;
        return;
      }
  ...
  // currently on the heal button
    ...
    if (pressing.up && !input_lock.up) {
      ...
      if (avatar.spellbook >= 3) {
        action.select_pos = BUTTON_POS_ICE;
        input_lock.down = true;
        redraw = true;
        return;
      }
  ...
  // currently on the FIRE button
  else if (action.select_pos == BUTTON_POS_FIRE) {
    if (pressing.up && !input_lock.up) {
      if (gamestate == STATE_COMBAT) {
        action.select_pos = BUTTON_POS_RUN;
        input_lock.up = true;
  ...
  // currently on the ICE button
  else if (action.select_pos == BUTTON_POS_ICE) {
    if (pressing.up && !input_lock.up) {
      action.select_pos = BUTTON_POS_HEAL;
      input_lock.up = true;
      redraw = true;
...
function action_render() {
  ...
  if (avatar.spellbook >= 2) action_render_button(3, BUTTON_POS_FIRE);
  if (avatar.spellbook >= 3) action_render_button(6, BUTTON_POS_ICE);
