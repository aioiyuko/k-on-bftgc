...
function combat_logic_input() {
  ...
  else if (action_checkuse(BUTTON_POS_FIRE) && avatar.mp > 0 && avatar.spellbook >= 2) {
    power_fire();
	used_action = true;
  }
  else if (action_checkuse(BUTTON_POS_ICE) && avatar.mp > 0 && avatar.spellbook >= 3) {
    power_ice();
	used_action = true;
  }
  else if (action_checkuse(BUTTON_POS_LIGHT) && avatar.mp > 0 && avatar.spellbook >= 4) {
    power_light();
	used_action = true;
  }
...
function combat_determine_reward() {
  ...
  combat.reward_result = "+" + gold_reward + " yen!";
...
function combat_render_defeat() {
  ...
  bitfont_render("Final Yen: " + avatar.gold, 158, 100, JUSTIFY_RIGHT);
}
